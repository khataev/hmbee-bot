...
- **Context**: Весь код команд `sync` и `apply` (валидация, оркестрация, вызовы клиентов) сосредоточен в `src/index.ts`. Параллельно `src/apply/` содержит и логику preview/нормализации, и вспомогательные функции самой команды, без чёткой границы между ними. Изменение `sync-apply-cache-hardening` добавило в необтестированный `apply`-action ещё один шаг оркестрации (форсированный refresh HM-кэша перед skip-pass); тестируемые куски (`refreshCache` в `src/hmbee/cache.ts`, `writeSyncOutput`/`clearPreviousSyncFiles` в `src/sync/cleanup.ts`) вынесены в отдельные модули, но сама последовательность вызовов внутри `apply` остаётся непокрытой тестами.
- **Goal**:
