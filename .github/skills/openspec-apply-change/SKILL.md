...
6. **Ask execution mode**
   ...
   Use the **AskUserQuestion tool** to ask how to proceed. Skip only if: the user already indicated a preference in the current conversation, or only one top-level task remains.
...
7. **Implement tasks**
   ...
   **If mode is "one top-level task at a time":**
   ...
   - Wait for the user to explicitly continue (e.g., by calling this skill again)
