...
- [x] 4.1 Добавить в `TRANSACTION-RULES.md` строки для `PaymentClaim` (сценарий, `included`/`excluded`, ссылки на тест и фикстуры) и обновить упоминания перенесённых правил категории
- [x] 4.2 Прогнать `npm run check` и весь набор `preview-*` тестов — всё зелёное
