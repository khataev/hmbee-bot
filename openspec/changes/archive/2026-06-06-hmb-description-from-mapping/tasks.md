...
- [x] 8.1 Добавить в Zod-схему `categoryMapping` поле `ignored: { mcc: string[]; title: string[] }` с дефолтом `{ mcc: [], title: [] }`
- [x] 8.2 При загрузке конфига включать `ignored.mcc` и `ignored.title` (как `RegExp[]`) в проверку auto-skip
- [x] 8.3 Реализовать ввод `i` / `im` / `it`:
...
- [x] 8.4 Обновить подсказку в консоли: добавить `i(gnore)` в список допустимых команд
- [x] 8.5 Прогнать `npm run check`, убедиться что все тесты проходят
