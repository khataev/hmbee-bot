...
Меняем `AppConfigSchema.sources` и `ResolvedAppConfigSchema.sources` на `z.record(bankName, BankConfigSchema)`. `tochka` остаётся обязательным (его `bankBic` и `typeCodes` нужны классификатору и депозитной эвристике), остальные банки опциональны.

- **Почему не оставить явные ключи `{ tochka, sber, tinkoff }`:** новый банк требовал бы правки схемы. `z.record` принимает любой набор. Tochka вытаскиваем по ключу `"tochka"` для bankBic/typeCodes/депозитов.
...
- **Tochka-обязательность** → классификатор и депозиты завязаны на `sources.tochka`. *Mitigation:* `tochka` остаётся required в схеме; отсутствие — ошибка валидации конфига.
...
1. Расширить схемы (`AppConfigSchema`, `ResolvedAppConfigSchema`) до record-формы с обязательным `tochka`.
