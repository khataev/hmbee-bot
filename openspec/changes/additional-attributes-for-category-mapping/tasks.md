...
- [x] 4.1 Добавить примеры правил в `config/sources.example.json` под `hmbee.categoryMapping.rules` (purpose "смс-информирование" и "лицензионного вознаграждения" → "Банки / Периодические списания")
- [x] 4.2 Добавить реальные правила в `config/sources.json` при необходимости (по согласованию)

## 5. Тесты

- [x] 5.1 Тест: правило по подстроке `purpose` для `PaymentWrittenOff` "смс-информирование" → "Банки / Периодические списания"
- [x] 5.2 Тест: правило по подстроке `purpose` для `PaymentWrittenOff` "лицензионного вознаграждения" → "Банки / Периодические списания"
- [x] 5.3 Тест: правило по `phoneNumber` для `SbpB2CPayment`
- [x] 5.4 Тест: правило по `cardPanPart` для `CardTransactionInfo`
- [x] 5.5 Тест приоритета: правило перехватывает запись, совпадающую и с title-паттерном
- [x] 5.6 Тест: первое совпавшее правило побеждает при нескольких совпадениях
- [x] 5.7 Тест: некорректный regex в `matches` не ломает резолюцию (fallback на следующий слой)
- [x] 5.8 Тест config: конфиг без `rules` валиден (`rules = []`); конфиг с `rules` парсится
- [x] 5.9 Добавить фикстуры записей при необходимости по образцу `src/apply/preview/fixtures/` (новые фикстуры не потребовались — существующие расширяются inline полями `purpose`/`cardPanPart`)
