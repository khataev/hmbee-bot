...
- [ ] 1.1 Реализовать zero-dependency декодер mozLz4: проверка сигнатуры `mozLz40\0`, чтение `uint32 LE` размера, декомпрессия raw LZ4-блока
- [ ] 1.2 Ошибка с понятным сообщением при неверной сигнатуре
- [ ] 1.3 Юнит-тесты декодера (валидный mozLz4 → JSON; битая сигнатура → ошибка)

## 2. Чтение Firefox sessionstore

- [ ] 2.1 Определить путь единственного профиля из `profiles.ini` (без конфигурации выбора)
- [ ] 2.2 Выбрать файл `recovery.jsonlz4`, при отсутствии/повреждении — `recovery.baklz4`; читать копию файла, не оригинал; при отсутствии обоих — ошибка «открой Firefox с залогиненной Точкой»
- [ ] 2.3 Декодировать через модуль из группы 1 и распарсить JSON sessionstore
- [ ] 2.4 Рекурсивно собрать cookie-объекты (по наличию полей `host`/`name`/`value`), устойчиво к структуре
- [ ] 2.5 Юнит-тесты на зафиксированном (санитизированном) фрагменте sessionstore
