...
- [x] 1.1 Изменить `loadSyncFiles` в `src/apply/preview/loader.ts`: возвращать явный тип с записями и распарсенными `from`/`to` из имени файла (`<from>_<to>.json`) вместо `TochkaSyncRecord[]`
- [x] 1.2 Добавить явную ошибку при некорректном/непарсящемся имени sync-файла
- [x] 1.3 Обновить потребителей `loadSyncFiles` в `src/index.ts` под новый контракт
- [x] 1.4 Обновить/добавить юнит-тесты loader на разбор `from`/`to` и на некорректное имя
...
- [x] 3.1 В команде `sync` (`src/index.ts`) перед записью нового файла удалять прочие `*.json` в `sync/<source>/` (только файловый режим; скоуп строго на подкаталог источника)
- [x] 3.2 Не трогать директорию при `--stdout`
- [x] 3.3 Добавить тесты: замена старого файла оставляет ровно один; `--stdout` не трогает директорию; `sync/hmbee/` не затронут
...
- [x] 5.1 Убрать опцию `--update-hmbee-cache` и связанный блок обновления кэша из команды `sync` в `src/index.ts`
- [x] 5.2 Удалить/переиспользовать ставший неиспользуемым код обновления кэша в контексте sync
- [x] 5.3 Обновить тесты sync под удалённый флаг
