...
## Purpose
Автоматическое получение свежих session-cookie для банковских адаптеров из локального браузера (Firefox sessionstore) через шов `CredentialProvider`, с доменной фильтрацией, дедупликацией и fallback на переменные окружения. Устраняет ручной перенос cookie из DevTools в `.env`.
